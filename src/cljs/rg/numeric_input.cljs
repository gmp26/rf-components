...
(defn numeric-input [{:keys [key]}]
  [:input {:type      "text"
           :value     0
           :on-change #(rf/dispatch [:age/change %])}])

#_(defn error? [val] (or (nil? val) (= "" val) (js/isNaN val)))

#_(defn handle-numeric-input [onChange e]
...
#_(defn handle-inc [val onChange min max step]
  (when (not= 0 step)
    (let [val-1 (if (= val "")
                  (if (pos? step)
                    (dec min)
